...
// Guard: only signed-in admins land here. Anyone else gets bounced back
// to the waitlist takeover at /.
function DashboardGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    if (!isSignedIn || !user || !isAdmin(user.id)) {
      router.replace("/");
    }
  }, [isLoaded, isSignedIn, user, router]);
  if (!isLoaded || !isSignedIn || !user || !isAdmin(user.id)) {
    return (
