...
h1{font-size:28px;font-weight:900;background:linear-gradient(135deg,#a78bfa,#f472b6);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
function draw(){
  ...
  ctx.beginPath();ctx.arc(food.x*GRID+GRID/2-2,food.y*GRID+GRID/2-2,3,0,Math.PI*2);ctx.fill();
  // snake
  snake.forEach((s,i)=>{
...
h1{font-size:30px;font-weight:900;color:#818cf8}
...
#feedback{font-size:20px;font-weight:700;height:28px}
...
<div class="card" id="game-card" style="display:none">
  <div id="meta">
    <span>✅ <span id="correct">0</span></span>
    <span>⏱ <span id="timeleft">30</span>s</span>
    <span>❌ <span id="wrong">0</span></span>
  </div>
...
function startGame(){
  ...
  timer=setInterval(()=>{
    ...
    if(timeLeft<=0){
      ...
      document.getElementById('resultEmoji').textContent=pct>=.8?'🏆':pct>=.5?'🎉':'💪';
...
h1{font-size:28px;font-weight:900;color:#38bdf8}
...
.opt{background:#0a1628;border:2px solid #1e3a5f;color:#cbd5e1;padding:13px 16px;border-radius:12px;width:100%;font-size:15px;cursor:pointer;text-align:left;margin-bottom:8px;transition:all .15s}
.opt:hover:not(:disabled){border-color:#38bdf8;background:#0f2f52}
...
#nextBtn{background:linear-gradient(135deg,#0284c7,#7c3aed);color:#fff;border:none;padding:12px 28px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;display:none;margin-top:8px}
button.primary{background:linear-gradient(135deg,#0284c7,#7c3aed);color:#fff;border:none;padding:14px 36px;border-radius:14px;font-size:17px;font-weight:700;cursor:pointer}
</style>
</head>
<body>
<h1>🧠 Trivia</h1>
...
<div class="card" id="result-card" style="display:none;text-align:center">
  <div id="resultEmoji" style="font-size:52px;margin-bottom:12px"></div>
...
function showResult(){
  ...
  document.getElementById('resultEmoji').textContent=p>=.9?'🏆':p>=.7?'🎉':p>=.5?'😊':'💪';
